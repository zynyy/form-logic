import { RecursionField, Schema, useField } from '@formlogic/render-core-vue2';
import { VNode, h } from 'vue';

const useTabPaneSchemaBtn = (schema: Schema, groupCode: string): VNode[] | null => {
  const field = useField();

  const btn = schema.reduceProperties((addition: VNode[], curSchema) => {
    const { name } = curSchema;

    if (
      name &&
      name === 'tabsPaneButtons' &&
      ['editable', 'disabled'].includes(curSchema['x-pattern'])
    ) {
      return addition.concat(
        h(RecursionField, {
          attrs: {
            schema: curSchema,
            name,
            key: name,
            basePath: field.value.address.concat(groupCode),
          },
        }),
      );
    }
    return addition;
  }, []);

  return btn.length ? btn : null;
};

export default useTabPaneSchemaBtn;
