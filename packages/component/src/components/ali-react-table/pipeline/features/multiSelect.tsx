...
import { safeGetRowKey } from '@/components/ali-react-table/utils/safe';
...
export function multiSelect(opts: MultiSelectFeatureOptions = {}) {
  return function multiSelectStep(pipeline: TablePipeline) {
    ...
    const value: string[] =
      opts.value ?? pipeline.getStateAtKey(stateKey)?.value ?? opts.defaultValue ?? [];
    const lastKey: string =
      opts.lastKey ?? pipeline.getStateAtKey(stateKey)?.lastKey ?? opts.defaultLastKey ?? '';
    const onChange: MultiSelectFeatureOptions['onChange'] = (nextValue, key, keys, action) => {
      opts.onChange?.(nextValue, key, keys, action);
      pipeline.setStateAtKey(stateKey, { value: nextValue, lastKey: key });
    };
