...
import { VerticalRenderRange } from '@/components/ali-react-table/art-table/interfaces';
import { OVER_SCAN_SIZE } from '@/components/ali-react-table/utils/constants';
...
export function makeRowHeightManager(initRowCount: number, estimatedRowHeight: number) {
  ...
  function getRenderRange(offset: number, maxRenderHeight: number, rowCount: number) {
    ...
    function overscanUpwards(topIndex: number, topBlank: number) {
      let overscanSize = 0;
      let overscanCount = 0;
      while (overscanCount < topIndex && overscanSize < OVER_SCAN_SIZE) {
        overscanCount += 1;
    ...
    function overscanDownwards(bottomIndex: number, bottomBlank: number) {
      let overscanSize = 0;
      let overscanCount = 0;
      while (overscanCount < rowCount - bottomIndex && overscanSize < OVER_SCAN_SIZE) {
