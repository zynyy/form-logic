...
import { parse, SFCBlock, compileTemplate, compileScript } from 'vue2-compiler-sfc';
...
function injectRender(script: string, render: string) {
  ...
  render = render.replace('var render = function render', `function ${RENDER_FN}`);
...
export async function compileSfcVue2(filePath: string): Promise<any> {
  ...
  const hasScoped = styles.some((s: any) => s.scoped);
