import { DragMoveEvent, DragStartEvent, DragStopEvent, ViewportScrollEvent } from '@/events';
import { ClosestPosition, CursorDragType, CursorType, Engine, TreeNode } from '@/models';
      ...
      if (dragNodes.length && closestNode && closestDirection) {
        if (
          ...
          if (closestNode.allowAppend(dragNodes)) {
            selection.batchSafeSelect(
              closestNode.append(...TreeNode.filterDroppable(dragNodes, closestNode)),
            );
          ...
          if (closestNode.allowAppend(dragNodes)) {
            selection.batchSafeSelect(
              closestNode.prepend(...TreeNode.filterDroppable(dragNodes, closestNode)),
            );
            moveHelper.dragDrop({ dropNode: closestNode });
          }
        }
