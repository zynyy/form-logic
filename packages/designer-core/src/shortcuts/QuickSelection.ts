...
export const SelectPrevNode = new Shortcut({
  ...
  handler(context) {
    ...
    if (operation) {
      ...
      const selectedNode = selection.last ? tree.findById(selection.last) : undefined;
      if (selectedNode) {
...
export const SelectNextNode = new Shortcut({
  ...
  handler(context) {
    ...
    if (operation) {
      ...
      const selectedNode = selection.last ? tree.findById(selection.last) : undefined;
      if (selectedNode) {
        const nextNode = selectedNode.firstChild ? selectedNode.firstChild : selectedNode.next;
        if (nextNode) {
