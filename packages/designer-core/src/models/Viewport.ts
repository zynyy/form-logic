...
export class Viewport {
  ...
  dragStartSnapshot: IViewportData | undefined;
  ...
  get isScrollRight() {
    if (this.isIframe) {
      return (
        this.width + this.contentWindow.scrollX >= this.contentWindow?.document?.body?.scrollWidth
      );
  ...
  get isScrollBottom() {
    if (this.isIframe) {
      ...
      return (
        this.height + this.contentWindow.scrollY >= this.contentWindow.document.body.scrollHeight
      );
    } else if (this.viewportElement) {
  ...
  cacheElements() {
    ...
    this.viewportRoot?.querySelectorAll(`*[${this.nodeIdAttrName}]`).forEach((element: any) => {
  ...
  getValidNodeElement(node: TreeNode): HTMLElement | undefined {
    const getNodeElement = (node: TreeNode): HTMLElement | undefined => {
      if (!node) return;
  ...
  getChildrenRect(node: TreeNode): Rect | undefined {
    ...
    return calcBoundingRect(
      node.children.reduce((buf: Rect[], child) => {
  ...
  getValidNodeLayout(node: TreeNode | null) {
