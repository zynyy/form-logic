...
import { isPlainObj, merge } from '@formily/shared';
...
import { IDesignerControllerProps, IDesignerLocales, IDesignerProps } from '@/interface';
import { mergeLocales } from '@/internals';
import { GlobalRegistry } from '@/registry';
import { each, isFn, uid } from '@/utils';
...
export type IMergeStrategy = 'deepMerge' | 'shallowMerge' | 'overwrite';
...
export class TreeNode {
  ...
  isSelfSourceNode: boolean = false;
  ...
  get descendants(): TreeNode[] {
    return this.children.reduce((buf: TreeNode[], node) => {
  ...
  triggerMutation<T>(event: any, callback?: () => T, defaults?: T): void | T {
    if (this.operation) {
      // @ts-ignore
  ...
  allowTranslate(): boolean {
    ...
    return !!(translatable?.x || translatable?.y);
  }
  ...
  setInitProps(props: Record<string, any>) {
    this.props = props || {};
  }

  setProps(props?: any, strategy: IMergeStrategy = 'deepMerge') {
  ...
  insertAfter(...nodes: TreeNode[]): TreeNode[] {
    ...
    if (parent?.children?.length) {
      ...
      return this.triggerMutation(
        ...
        () => {
          parent.children = parent.children.reduce((buf: TreeNode[], node) => {
            if (node === this) {
  ...
  insertChildren(start: number, ...nodes: TreeNode[]) {
    ...
    if (this.children?.length) {
      ...
      return this.triggerMutation(
        ...
        () => {
          this.children = this.children.reduce((buf: TreeNode[], node, index) => {
            if (index === start) {
              return buf.concat(newNodes).concat([node]);
            }
            return buf.concat([node]);
  ...
  static clone(nodes: TreeNode[] = []) {
    ...
    each(groups, (nodes, parentId) => {
      ...
      let insertPoint: TreeNode | undefined = lastNode;
      each(nodes, (node) => {
  ...
  static filterDraggable(nodes: TreeNode[] = []) {
    return nodes.reduce((buf: TreeNode[], node) => {
