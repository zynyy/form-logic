...
import { DragNodeEvent, DropNodeEvent } from '@/events/mutation';
import {
  IPoint,
  Rect,
...
export class MoveHelper {
  ...
  calcClosestPosition(point: IPoint, viewport: Viewport): ClosestPosition | null {
    ...
    const getValidParent = (node: TreeNode): TreeNode | null => {
      if (!node) {
        return null;
      }
  ...
  calcClosestNode(point: IPoint, viewport: Viewport): TreeNode | null {
    if (this.touchNode) {
      ...
      if (this.touchNode?.children?.length) {
        let minDistance = calcDistancePointToEdge(point, touchNodeRect);
  ...
  calcClosestRect(viewport: Viewport, closestDirection: ClosestPosition | null): Rect | null {
    const closestNode = this.closestNode;
    ...
    if (
      ...
      return viewport.getChildrenRect(closestNode) ?? null;
    } else {
      return closestRect ?? null;
    }
  }

  calcClosestOffsetRect(viewport: Viewport, closestDirection: ClosestPosition | null): Rect | null {
    const closestNode = this.closestNode;
    if (!closestNode || !closestDirection) {
      return null;
    }
    ...
    if (
      ...
      return viewport.getChildrenOffsetRect(closestNode) ?? null;
    } else {
      return closestRect ?? null;
    }
  }
