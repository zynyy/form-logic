...
import { PrimaryKey } from '@/components/ali-react-table/art-table/interfaces';
import { ArtTableProps } from '@/components/ali-react-table/art-table';

type RowPropsGetter = ArtTableProps['getRowProps'];
...
export class TablePipeline {
  ...
  private useTablePipelineMap = new Map();
  ...
  private readonly setState: (fn: (prevState: any) => any) => any;
  ...
  setStateAtKey(stateKey: string, partialState: any) {
    this.setState((prev: any) => {
      return { ...prev, [stateKey]: partialState };
    });
  }
  ...
  use(step: (pipeline: this) => this) {
    this.useTablePipelineMap.set(`usePip${this.useTablePipelineMap.size}`, step);

    return step(this);
  }

  useStep() {
    this.useTablePipelineMap.forEach((value) => {
      value(this);
    });
