...
import { patchCompile, shallowCompile } from '@formily/json-schema/lib/compiler';
import { autorun, observable, untracked } from '@formily/reactive';
import { each, isArr, isFn, isPlainObj, isStr, lazyMerge, reduce, toArr } from '@formily/shared';

import { patchSchemaCompile } from './compiler';

const FieldEffects: Record<string, any> = {
  onFieldInit,
  onFieldMount,
  onFieldUnmount,
  onFieldValueChange,
...
const getDependencyValue = (field: Field, pattern: string, property?: string) => {
  ...
  if (isArr(dependencies)) {
    ...
    dependencies.forEach((pattern) => {
      if (isStr(pattern)) {
        ...
        const nextPattern = pattern as Record<string, any>;

        if (nextPattern.name && nextPattern.source) {
          results[nextPattern.name] = getDependencyValue(
            field,
            nextPattern.source,
    ...
    return reduce(
      dependencies,
      (buf: Record<string, any>, pattern, key) => {
...
const setSchemaFieldState = (options: IFieldStateSetterOptions, demand = false) => {
...
const getBaseScope = (field: Field, options: ISchemaTransformerOptions = {}) => {
  const $observable = (target: any, deps?: any[]) => autorun.memo(() => observable(target), deps);
